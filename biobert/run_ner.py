...
import collections
import os
from libs.biobert import modeling
from libs.biobert import optimization
from libs.biobert import tokenization
import tensorflow as tf
from tensorflow.python.ops import math_ops
from libs.biobert import tf_metrics
import pickle
import nltk
...
# temp fix by umesh. REVISIT
tf.flags.DEFINE_string("text", None, "Text to be tagged")
tf.flags.DEFINE_string("file", None, "File that contains assessments to be tagged")
...
#**
#* The below class is added by Umesh
#**
class CustomNerProcessor(NerProcessor):

    def get_labels(self):
        return ["B", "I", "O", "X", "[CLS]", "[SEP]"]
        #return ["O", "X", "[CLS]", "[SEP]", "B-test", "I-test", "B-problem", "I-problem", "B-treatment", "I-treatment"]

    def get_pred_examples(self, text, spacy_nlp):
        return self._create_example(
            self._read_data_text(text, spacy_nlp), "test")
    ...
    @classmethod
    def _read_data_text(cls, text, nlp):
        """
            Returns the ner for a given text
            :param text:
            :return:
            """
        lines = []
...
def file_based_input_fn_builder_nvidia(input_file, batch_size, seq_length, is_training, drop_remainder, hvd=None):
    name_to_features = {
...
def model_fn_builder(bert_config, num_labels, init_checkpoint=None, learning_rate=None,
    ...
    # default argument values are added by umesh
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            # def metric_fn(label_ids, logits):
                ...
                #changed by umesh
                no_lables_w_off = len(NerProcessor().get_labels()) + 1
                precision = tf_metrics.precision(label_ids, predictions, no_lables_w_off, [1, 2], average="macro")
                recall = tf_metrics.recall(label_ids, predictions, no_lables_w_off, [1, 2], average="macro")
                f = tf_metrics.f1(label_ids, predictions, no_lables_w_off, [1, 2], average="macro")
